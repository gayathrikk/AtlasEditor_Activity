...
public class AtlasEditor_Activity {
	...
	.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[@title='Atlas Editor']")));
	if (HdElement != null && HdElement.isDisplayed()) {
	HdElement.click();
	System.out.println("Atlas Editor Icon clicked successfully");
	} else {
	...
	java.util.List<WebElement> locationElements = driver
	.findElements(By.xpath("(//td[text()='SoftwareTesting Team
			       '])[1]/following-sibling::td[2]"));
	String actualFileName = locationElements.get(0).getText();
	Thread.sleep(2000);
